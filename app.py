...
def delete_file():
    ...
    if file_name:
        file_path = os.path.join('tarifas', file_name)

        print(f"Intentando eliminar el archivo: {file_path}")

        # Obtener la extensión del archivo
        _, file_extension = os.path.splitext(file_path)

        if os.path.exists(file_path) and file_extension.upper() == '.CSV':
            os.remove(file_path)
            return f"Archivo '{file_name}' eliminado correctamente."
        else:
            return f"No se encontró el archivo '{file_name}' en la carpeta 'tarifas' o no es un archivo CSV. Ruta completa: {file_path}", 400
    else:
        return "Nombre de archivo no proporcionado en el formulario.", 400
